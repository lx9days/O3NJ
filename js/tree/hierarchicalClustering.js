...
module.exports = function extend() {
	var options, taxa, src, copy, copyIsArray, clone,
		target = arguments[0],
		i = 1,
			...
			for (taxa in options) {
				...
				if (target !== copy) {
					...
					if (deep && copy && (isPlainObject(copy) || (copyIsArray = isArray(copy)))) {
						...
						target[taxa] = extend(deep, clone, copy);
  ...
  clust._buildTree = function() {
    ...
    data.forEach(function(d, ndx) {
      d._distances = data.map(function(compareTo) {
        // console.log(d);
        // console.log(d[posKey]);
        let dPos = [d.x, d.y];
        let compareToPos = [compareTo.x, compareTo.y];
        return distanceFn(dPos, compareToPos);
      });
